...
export function isNodeNamedDeclaration(node: ts.Node): node is ts.NamedDeclaration {
  return namedDeclarationKinds.includes(node.kind);
}

export function hasNodeModifier(node: ts.Node, modifier: ts.SyntaxKind): boolean {
  const modifiers = getModifiers(node);
  return Boolean(modifiers?.some((nodeModifier: ts.Modifier) => nodeModifier.kind === modifier));
}
...
export function recreateRootLevelNodeWithModifiers(node: ts.Node, modifiersMap: ModifiersMap, newName?: string, keepComments = true): ts.Node {
...
function recreateRootLevelNodeWithModifiersImpl(node: ts.Node, modifiersMap: ModifiersMap, newName?: string): ts.Node {
  ...
  if (ts.isExportDeclaration(node)) {
    type Ts53CompatExportDeclaration = {
  ...
  if (ts.isImportDeclaration(node)) {
    type Ts53CompatImportDeclaration = {
...
export function getRootSourceFile(program: ts.Program, rootFileName: string): ts.SourceFile {
  if (!program.getRootFileNames().includes(rootFileName)) {
