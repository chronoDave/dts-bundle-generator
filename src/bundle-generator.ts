import type { SourceFileExport } from './helpers/typescript.ts';
import type { ModuleCriteria, ModuleInfo } from './module-info.ts';
import type { ModuleImportsSet, OutputInputData, StatementSettings } from './generate-output.ts';

import * as ts from 'typescript';

import { compileDts } from './compile-dts.ts';
import { TypesUsageEvaluator } from './types-usage-evaluator.ts';
import {
  ExportType,
  getActualSymbol,
...
import { generateOutput } from './generate-output.ts';
import {
  normalLog,
  verboseLog,
  warnLog
} from './logger.ts';
import { CollisionsResolver } from './collisions-resolver.ts';
...
export function generateDtsBundle(entries: readonly EntryPointConfig[], options: CompilationOptions = {}): string[] {
  ...
  return entries.map((entryConfig: EntryPointConfig) => {
    ...
    type CollectingResult = {
      statements: ts.Statement[];
    } & Omit<OutputInputData, 'statements'>;
    ...
    function handleExportDeclarationFromRootModule(exportDeclaration: ts.ExportDeclaration): void {
      function handleExportStarStatement(exportStarStatement: ts.ExportDeclaration, visitedSymbols = new Set<ts.Symbol>()): void {
      ...
      type ExportingExportStarExport = {
    ...
    function addImport(statement: ts.DeclarationStatement | ts.SourceFile): void {
      forEachImportOfStatement(statement, (imp: ImportOfStatement, referencedModuleInfo: ModuleInfo, importModuleSpecifier: string) => {
		...
		function isSymbolUsedByInlinedSymbols(symbol: ts.Symbol, predicate: (usedInSymbol: ts.Symbol) => boolean, visitedSymbols = new Set<ts.Symbol>()): boolean {
