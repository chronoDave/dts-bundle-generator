...
import { verboseLog } from './logger.ts';
...
export type ResolverIdentifier = {
  name: string;
...
export class CollisionsResolver {
  ...
  private readonly collisionsMap = new Map<string, Map<ts.Symbol, string>>();
  private readonly generatedNames = new Map<ts.Symbol, Set<string>>();
  ...
  /**
	 ...
	 */
  addTopLevelIdentifier(identifier: ts.Identifier | ts.DefaultKeyword): string {
