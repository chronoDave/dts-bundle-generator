...
export class TypesUsageEvaluator {
  ...
  private readonly nodesParentsMap = new Map<ts.Symbol, Set<ts.Symbol>>();

  private readonly usageResultCache = new Map<ts.Symbol, Map<ts.Symbol, boolean>>();
  ...
  private addUsagesForNamespacedModule(namespaceNode: ts.NamespaceImport | ts.NamespaceExport, moduleSpecifier: ts.StringLiteral, includeExports = true): void {
  ...
  private addExportsToSymbol(exports: ts.SymbolTable | undefined, parentSymbol: ts.Symbol, visitedSymbols = new Set<ts.Symbol>()): void {
