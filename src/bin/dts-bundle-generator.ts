...
import type { BundlerConfig, ConfigEntryPoint } from '../config-file/load-config-file.ts';

import * as path from 'path';
import * as ts from 'typescript';
import * as yargs from 'yargs';

import { loadConfigFile } from '../config-file/load-config-file.ts';
import { generateDtsBundle } from '../bundle-generator.ts';
import { checkProgramDiagnosticsErrors } from '../helpers/check-diagnostics-errors.ts';
import { getCompilerOptions } from '../get-compiler-options.ts';
import {
  enableNormalLog,
  enableVerbose,
  errorLog,
...
function generateOutFileName(inputFilePath: string): string {
  ...
  return path.normalize(path.join(inputFilePath, '..', inputFileName + '.d.ts'));
}
